...
def read_image(path):
    ...
    png_path = root + '.png'
    if ext.lower() in ['.tif', '.tiff'] and os.path.exists(png_path):
        path = png_path
    return np.copy(mpimg.imread(path))
...
def init_centroids(num_clusters, image):
    ...
    picked = np.random.choice(pixels.shape[0], num_clusters, replace=False)
    centroids_init = pixels[picked].astype(float)
    # *** END YOUR CODE ***
...
def update_centroids(centroids, image, max_iter=30, print_every=10):
    ...
    # *** START YOUR CODE ***
    pixels = image.reshape(-1, image.shape[-1]).astype(float)
    new_centroids = centroids.astype(float).copy()

    for i in range(max_iter):
        old_centroids = new_centroids.copy()
        dists = np.sum((pixels[:, None, :] - old_centroids[None, :, :]) ** 2, axis=2)
        closest = np.argmin(dists, axis=1)

        for j in range(old_centroids.shape[0]):
            mine = pixels[closest == j]
            if len(mine) > 0:
                new_centroids[j] = np.mean(mine, axis=0)

        if (i + 1) % print_every == 0:
            print('[INFO] Iteration %d done' % (i + 1))
        if i >= 29 and np.allclose(old_centroids, new_centroids):
            break
    # *** END YOUR CODE ***
...
def update_image(image, centroids):
    ...
    shape = image.shape
    pixels = image.reshape(-1, shape[-1]).astype(float)
    dists = np.sum((pixels[:, None, :] - centroids[None, :, :]) ** 2, axis=2)
    closest = np.argmin(dists, axis=1)
    image = centroids[closest].reshape(shape)

    if np.issubdtype(old_dtype, np.integer):
        image = np.clip(image, 0, 255).astype(old_dtype)
    else:
        image = np.clip(image, 0, 1).astype(old_dtype)
    # *** END YOUR CODE ***
...
def main(args):
    ...
    # Load small image
    image = read_image(image_path_small)
    ...
    # Load large image
    image = read_image(image_path_large)
