...
def forward_prop(data, one_hot_labels, params):
    ...
    scores = np.dot(hidden, params['W2']) + params['b2']
    output = softmax(scores)
    cost = -np.mean(np.sum(one_hot_labels * np.log(output + 1e-12), axis=1))
...
def backward_prop(data, one_hot_labels, params, forward_prop_func):
    ...
    """
    # *** START CODE HERE ***
    hidden, output, cost = forward_prop_func(data, one_hot_labels, params)
    n = data.shape[0]

    d2 = (output - one_hot_labels) / n
    gradW2 = np.dot(hidden.T, d2)
    gradb2 = np.sum(d2, axis=0)

    dhidden = np.dot(d2, params['W2'].T)
    d1 = dhidden * hidden * (1 - hidden)
...
def gradient_descent_epoch(train_data, one_hot_train_labels, learning_rate, batch_size, params, forward_prop_func, backward_prop_func):
    ...
    for start in range(0, n, batch_size):
        ...
        labels = one_hot_train_labels[start:end]
...
def read_data(images_file, labels_file):
    if (not os.path.exists(images_file)) and os.path.exists(images_file + '.gz'):
        images_file = images_file + '.gz'
    if (not os.path.exists(labels_file)) and os.path.exists(labels_file + '.gz'):
        labels_file = labels_file + '.gz'
...
def run_train_test(name, all_data, all_labels, backward_prop_func, num_epochs, plot=True):
    ...
    np.savez('./' + name + '_params.npz', **params)
