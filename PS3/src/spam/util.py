...
def load_bert_encoding(encoding_path):
    """Load the BERT encoding matrix for a dataset.

    Args:
        encoding_path: Path to the ``.npz`` encoding file.

    Returns:
        matrix: The encoding matrix for the dataset.
    """
    with np.load(encoding_path) as data:
        if 'arr' in data:
            return data['arr']
